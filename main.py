...
# Load and authenticate with Google
try:
    ...
    st.error("Failed to authenticate with Google Sheets API. Check your credentials.")
    st.stop()
...
def connect_google_sheet():
    """Connect to the Google Sheet and return the sheet object."""
    try:
...
def get_or_create_monthly_sheet(sheet_obj):
    """Retrieve or create the MonthlyPlan worksheet."""
    try:
...
def landing_page():
    ...
    st.write("Choose an action below:")
    col1, col2, col3 = st.columns(3)
    if col1.button("Your Task"):
        with st.spinner("Opening Your Task..."):
            time.sleep(2)
        st.session_state.page = "your_task"
        safe_rerun()
    if col2.button("Edit Team's Task"):
        with st.spinner("Opening Edit Team's Task..."):
            time.sleep(2)
        st.session_state.page = "edit_team"
        safe_rerun()
    if col3.button("Schedule Monthly Tasks"):
        with st.spinner("Opening Monthly Tasks..."):
            time.sleep(2)
        st.session_state.page = "schedule_monthly"
        safe_rerun()
...
def page_your_task():
    ...
    department = col3.selectbox("Department", ["STARWOX", "ZUMMEY", "SAFEBOX ENERGY", "CREATIVE", "EXECUTIVE ASSISTANTS", "DEVELOPERS"])
    project = col4.text_input("Project")
    date_val = col5.date_input("Date")

    st.markdown("### Tasks (1 - 6)")
    tasks = [st.text_input(f"Task {i+1}") for i in range(6)]
    ...
    if st.button("Submit Tasks"):
        ...
        if not all([name.strip(), email.strip(), department.strip(), project.strip()]):
            ...
            try:
                sheet1.append_row([str(date_val), name, email, department, project] + tasks)
                st.success("Tasks submitted successfully!")
            except Exception as e:
                st.error("Error appending row to the sheet.")
                st.error(e)
...
if st.session_state.page == "landing":
    ...
    page_your_task()
# Additional pages (edit_team, schedule_monthly) should be implemented similarly
