...
# --- Load Configuration from Streamlit secrets (with section lookups) ---
CONFIG = {
    # general section
    "APP_USERNAME":    st.secrets["general"].get("APP_USERNAME",    os.getenv("APP2_USERNAME")),
    "APP_PASSWORD":    st.secrets["general"].get("APP_PASSWORD",    os.getenv("APP2_PASSWORD")),
    "GOOGLE_SHEET_ID": st.secrets["general"].get("GOOGLE_SHEET_ID", os.getenv("GOOGLE_SHEET_ID", "")),

    # smtp section
    "SMTP_SERVER":     st.secrets["smtp"].get("SMTP_SERVER",     os.getenv("SMTP_SERVER", "")),
    "SMTP_PORT":       st.secrets["smtp"].get("SMTP_PORT",       os.getenv("SMTP_PORT", "")),
    "SMTP_USER":       st.secrets["smtp"].get("SMTP_USER",       os.getenv("SMTP_USER", "")),
    "SMTP_PASSWORD":   st.secrets["smtp"].get("SMTP_PASSWORD",   os.getenv("SMTP_PASSWORD", "")),
}
...
with st.sidebar:
    ...
    if not st.session_state.logged_in:
        ...
        if st.button("Login"):
            if (
                user_input.strip() == (CONFIG["APP_USERNAME"] or "").strip()
                and pass_input.strip() == (CONFIG["APP_PASSWORD"] or "").strip()
            ):
                st.session_state.logged_in = True
                st.success("Logged in!")
            else:
    ...
    st.subheader("Guidelines")
    st.info(
        "1. Enter your Name, Email, and Date below.\n"
        "2. Click 'Fetch Tasks' to retrieve tasks from the Google Sheet.\n"
        "3. Review the tasks, add your supervisor comment, and click 'Send Comments' to email your comment."
    )
...
if st.session_state.logged_in:
    ...
    if st.session_state.get("form_submitted", False):
        ...
        try:
            ...
            st.error("Error loading Google credentials.")
            st.error(e)
            st.stop()
        ...
        try:
            rows = sheet.get_all_values()
            for row in rows[1:]:
                if len(row) >= 2 and row[0].strip() == landing_date and row[1].strip().lower() == landing_name.lower():
                    matched_row = row
                    break
        except Exception as e:
            st.error("Error fetching data from the sheet.")
